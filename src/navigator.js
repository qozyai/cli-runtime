...
function normalizeDecision(value) {
  ...
  if (action === "submit_text") {
    ...
    if (!text || text.length > 256 || /[\r\n]/.test(text)) throw new Error("navigator returned invalid text");
...
class Navigator {
  constructor({ config, eventStore, fetchImpl = fetch }) {
    this.config = config;
  ...
  get enabled() {
    return Boolean(this.config.navigator.url);
  }
